...
import com.iot.container.MessageAcceptor;
    ...
    public void accept(String topic, byte[] message)
    {
        ...
        if(bean != null && bean instanceof ITopicHandler)
        {
            ...
            try
            {
                ...
                /**
                 * 如果有返回值，并且topic的路径设计有返回写数据topic，就pub一下结果
                 * 比如  /test/t/1
                 * /test 表示服务处理接收topic， /t/1 表示结果返回的topic
                 */
                if(result != null && topicArray.length > 1)
                {
                   applicationContext.getBean(RsocketClientSession.class).pub(getReturnTopic
                        (topicArray),result);
                }
            }
            catch (Exception e)
